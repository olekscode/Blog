source('theme.R')
source('colors.R')

library(ggplot2)
library(ggthemes)
library(grid)
library(gridExtra)
library(scales)

line_color <- blue

line_func <- function(slope, intercept)
  function(area)
    slope * area + intercept

error_lines <- function(data, pred_func)
  geom_segment(aes(
    x=area,
    xend=area,
    y=price,
    yend=pred_func(area)),
    color=red)

error_squares <- function(data, pred_func) {
  squares <- c()
  
  for (i in seq(nrow(data))) {
    area <- data$area[i]
    true_price <- data$price[i]
    pred_price <- pred_func(area)
    
    ymin <- min(true_price, pred_price)
    ymax <- max(true_price, pred_price)
    xmin <- area
    xmax <- area + ((ymax - ymin) / 10)
    
    square <- ggplot2::annotate(
      "rect",
      xmin=xmin,
      xmax=xmax,
      ymin=ymin,
      ymax=ymax,
      fill=red, alpha=0.1)
    
    squares <- c(squares, square)
  }
  
  return(squares);
}

draw_regression <- function(slope, intercept, title)
  ggplot(data, aes(x=area, y=price)) +
    geom_point() +
    geom_abline(intercept=intercept, slope=slope, color=line_color, lwd=.8) +
    error_squares(data, line_func(slope, intercept)) +
    error_lines(data, line_func(slope, intercept)) +
    ggtitle(title) +
    labs(x="Area (sq. m)", y="Price (euros)") +
    theme_Publication() +
    theme(panel.grid.major=element_blank(), text=element_text(family="Avenir"))

n <- 10
areas <- runif(n) * 40 + 10
prices <- areas * 10 + 200 + rnorm(length(areas)) * 70

data <- data.frame(
  area = areas,
  price = prices)

model <- lm(price ~ area, data)

p1 <- draw_regression(-5, 700, "Bad Line")
p2 <- draw_regression(5, 350, "Better Line")
p3 <- draw_regression(model$coefficients[2], model$coefficients[1], "Best Line")

result <- grid.arrange(arrangeGrob(p1, p2, p3, ncol=3))

ggsave('../img/goodRegressionLine.png', plot=result, dpi=800)
